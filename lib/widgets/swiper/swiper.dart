        ...
        assert(
            !loop ||
                ((loop &&
                        ...
                        (indicatorLayout == PageIndicatorLayout.SCALE || indicatorLayout == PageIndicatorLayout.COLOR || indicatorLayout == PageIndicatorLayout.NONE)) ||
...
abstract class _SwiperTimerMixin extends State<Swiper> {
  ...
  void _startAutoplay() {
    ...
    if (widget.playNow) {
      Future.delayed(const Duration(milliseconds: 30)).then((value) => _controller.next(animation: true));
    }
  }
...
class _StackViewState extends _CustomLayoutStateBase<_StackSwiper> {
  ...
  void _updateValues() {
    if (widget.scrollDirection == Axis.horizontal) {
      ...
      offsets = widget.axisDirection == AxisDirection.left ? [-space, -space / 3 * 2, -space / 3, 0.0, _swiperWidth] : [_swiperWidth, 0.0, -space / 3, -space / 3 * 2, -space];
    } else {
