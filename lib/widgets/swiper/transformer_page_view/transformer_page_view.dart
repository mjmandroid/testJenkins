...
class TransformerPageController extends PageController {
  TransformerPageController({
    ...
    this.itemCount = 0,
    this.reverse = false,
  }) : super(initialPage: TransformerPageController._getRealIndexFromRenderIndex(initialPage, loop, itemCount, reverse), keepPage: keepPage, viewportFraction: viewportFraction);
...
class _TransformerPageViewState extends State<TransformerPageView> {
  ...
  Widget _buildItem(BuildContext context, int index) {
    return AnimatedBuilder(
        ...
        builder: (c, w) {
          final renderIndex = _pageController.getRenderIndexFromRealIndex(index);
          final child = widget.itemBuilder?.call(context, renderIndex) ?? const SizedBox.shrink();
          if (_size == null) {
            return child;
          }
